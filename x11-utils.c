...
bool x11_set_input_device_enabled(Display* dpy, const XIDeviceInfo devinfo, const bool enable)
{
	...
	if ((x11_atom_device_enabled_type == None) || (x11_atom_device_enabled_format == 0)) {
		...
		if (XIGetProperty(dpy, devinfo.deviceid, x11_atom_device_enabled, 0, 0, False, AnyPropertyType, &x11_atom_device_enabled_type, &x11_atom_device_enabled_format, &ignore_nitems, &ignore_bytes_after, &ignore_data)) {
...
bool x11_set_all_input_devices_enabled(Display* dpy, const bool enable)
{
	...
	if (!enable) for (int i = 0; i < ndev; ++i) {
		if (devinfo[i].use == XIMasterPointer || devinfo[i].use == XIMasterKeyboard || !devinfo[i].enabled)
			continue;
